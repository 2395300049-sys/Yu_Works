...
def looks_like_toc_entry_line(text):
    return bool(re.search(r'\.{4,}\s*\d+$', text))

def looks_like_numbered_toc_entry_with_page_suffix(text):
    return bool(re.search(r'\d+\s*$', text.strip()))

def looks_like_date_placeholder_line(text):
    return bool(re.search(r'年\s*月\s*日', text))

def looks_like_reference_entry_line(text):
    return bool(re.match(r'^\s*\[\d{1,3}\]', text) or re.match(r'^\s*\d{1,3}\.', text))
...
def _score_paragraph(para: Paragraph, sec_type: str,
    ...
    for anchor in anchors:
        ...
        if text_no_space == anchor_no_space:
            ...
            local_score = 2.0
            if text_no_space.endswith(anchor_no_space):
                local_score = 8.0
...
class DocTree:
    ...
    _RE_CAPTION_PREFIX = re.compile(
        ...
        r"[（(]\d+(?:[.\-]\d+)*[）)]|"
        r"\d+(?:[.\-]\d+)*)"
        r"(?=\s|[（(A-Za-z一-鿿]|$)", re.IGNORECASE
    )
    _RE_REFERENCE = re.compile(r'^\s*\[\d{1,4}\]\s*\S|^\s*\d{1,4}\.\s+\S')

    def get_refined_role(self, para_idx: int, text: str) -> str:
        section = self.get_section_for_paragraph(para_idx)
        if self._RE_CAPTION_PREFIX.match(text):
            return "caption"
        if self._RE_REFERENCE.match(text):
            if len(text) < 60:
                return "body"
            return "reference"
