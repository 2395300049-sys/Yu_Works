...
from .ir import InlineSpan, InlineType
from .inline_parser import parse_inline
from .line_spacing import apply_safe_picture_line_spacing
...
def write_spans(para, spans: list[InlineSpan], *,
                ...
                prefer_real_image: bool = False,
                global_stats=None, para_idx=-1,
                escapes_map=None) -> None:
    ...
    for span in spans:
        ...
        # catch-all: TEXT / BOLD / ITALIC / CODE 等全部 inline 类型
        run = para.add_run(span.text)
        set_no_proof(run)

        if en_font or cn_font:
    ...
    # 还原转义隐身符：必须在 OMML 转换前执行
    if escapes_map:
        from core.ai_normalizer import restore_escapes
        for run in para.runs:
            run.text = restore_escapes(run.text, escapes_map)

    # OMML 公式转换：段落渲染完毕后，将 $...$ / \(...\) 转为 Word 原生公式
    if _HAS_OMML:
